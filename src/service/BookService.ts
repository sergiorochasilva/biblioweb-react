...
export async function fetchRecentPublications(): Promise<Book[]> {
    ...
    //     }
    // });
    const response = await fetch("https://biblioweb.online:8080/libraries_books?library=1&fields=review");
    if (!response.ok) {
...
export async function fetchSearchResults(query: string): Promise<Book[]> {
    ...
    //     }
    // });
    const response = await fetch(`https://biblioweb.online:8080/libraries_books?library=1&search=${query}&fields=review`);
    if (!response.ok) {
